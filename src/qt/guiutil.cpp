...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("blackbitz"))
        return false;
...
{
    // Convert blackbitz:// to blackbitz:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("blackbitz://"))
    {
        uri.replace(0, 12, "blackbitz:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BlackBitz.lnk";
}
...
{
    return GetAutostartDir() / "blackbitz.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BlackBitz-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  blackbitz-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BlackBitz-Qt"));
